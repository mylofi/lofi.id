...
async function toMnemonic(ivBits) {
	try {
		...
		let ivBitArray = Array.from(new Uint32Array(ivBits.buffer)).flatMap(v =>
			Array.from(v.toString(2).padStart(32,"0"))
		);

		// compute the checksum bits and put them in a regular array
		let checksum = computeChecksum(ivBits);
		let checksumBitArray = Array.from(checksum.toString(2).padStart(ivBits.byteLength / 4,"0"));

		// split the bit arrays (`ivBitArray` + `checksumBitArray`)
		// into an array of 11-bit base10 numbers
		// (11 bits: 0 - 2047 in base10)
		let chunks = Array((ivBitArray.length + checksumBitArray.length) / 11).fill(0)
			.map((_,idx) => {
				var bitStr = (
...
async function fromMnemonic(words) {
	...
	try {
		...
		var ivBitArray = bitArray.slice(0,Math.ceil(((words.length - 1) * 11) / 8) * 8);
		var ivBits = new ArrayBuffer(ivBitArray.length / 8);
		var ivBits8 = new Uint8Array(ivBits);
		var ivBits32 = new Uint32Array(ivBits);
		Array(ivBits32.length).fill(0).forEach((_,idx) => {
			var bitStr = ivBitArray.slice(idx * 32,(idx + 1) * 32).join("");
